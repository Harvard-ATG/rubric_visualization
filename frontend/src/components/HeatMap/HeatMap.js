...
export const drawHeatMap = (id, data, totalAssessments) => {
  ...
  data.forEach((criterion, index) => {
    ...
    svg.selectAll()
      .data(criterion, function(d) {return d.criterion+':'+d.ratingDescription;})
      .enter()
      .append("rect")
      .attr("transform","translate(0," + (y.bandwidth()*(index) + ((index*3)*criteriaLength)) + ")")
      .attr("x", function(d) { return x(d.ratingDescription) + 10})
      .attr("y", function(d) { return y(d.criterion) + 10 })
